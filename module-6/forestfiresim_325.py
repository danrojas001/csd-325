    ...
    while True:  # Main program loop.
        ...
        for x in range(forest['width']):
            for y in range(forest['height']):
                ...
                elif forest[(x, y)] == LAKE:  # Keeps the lake unchanged
                    nextForest[(x, y)] = LAKE
                else:
...
def createNewForest():
    ...
    for x in range(WIDTH):
        # Finds the middle width of the forest +/- 2
        if math.floor(WIDTH / 2) - 2 <= x <= math.floor(WIDTH / 2) + 2:
            for y in range(HEIGHT):
                # Allows for empty space for tree growth on the north and south ends of lake
                if y == 0 or y == HEIGHT - 1:
                    forest[(x, y)] = EMPTY
                else:
                    forest[(x, y)] = LAKE  # Creates a lake
        else:
            for y in range(HEIGHT):
    ...
    for y in range(forest['height']):
        for x in range(forest['width']):
            if forest[(x, y)] == TREE:
                ...
                bext.fg('red')
                print(FIRE, end='')
            elif forest[(x, y)] == LAKE:  # Displays the lake
                bext.fg('blue')
                print(LAKE, end='')
